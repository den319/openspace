'use client'
import { FormTypeBookSlot } from '@openspace/forms/src/bookSlot'
import { loadStripe } from '@stripe/stripe-js'

import {
  CreateBookingInput,
  SearchGaragesQuery,
} from '@openspace/network/src/gql/generated'
...
import { toLocalISOString } from '@openspace/util/date'
import { useTotalPrice } from '@openspace/util/hooks/price'
...
export const createBookingSession = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      toast.error(`API request failed: ${response.status} ${errorText}`)
    ...
    toast.error(`Error creating booking session: ${error}`)
  }
}
