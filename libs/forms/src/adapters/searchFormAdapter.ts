import { SearchGaragesQueryVariables } from '@openspace/network/src/gql/generated'
...
import { useDebounce } from '@openspace/util/hooks/async'
...
function toISOWithSeconds(value?: string) {
  if (!value) return String(new Date())
...
export const useConvertSearchFormToVariables = () => {
  ...
  useEffect(() => {
    ...
    const dateFilter: SearchGaragesQueryVariables['dateFilter'] = {
      // start: toISOWithSeconds(startTime),
      // end: toISOWithSeconds(endTime),
      start: startTime,
      end: endTime,
    }
